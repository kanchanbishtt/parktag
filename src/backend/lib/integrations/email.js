...
// The premium year is ending.
//
// ── Why this says more than the WhatsApp template does ─────────────────────
//
// `parktag_trial_ending` is a UTILITY template, and it stayed utility partly
// because the price came OUT of it at submission. Meta's categories govern what
// a template may say; nobody governs an e-mail. So this is the version that can
// name the price, show what lapsing actually costs, and ask for the renewal.
//
// That division is the point of running both channels rather than treating
// e-mail as WhatsApp's understudy: the message people read carries the fact,
// and the message that can sell carries the offer.
export async function sendTrialEndingEmail(env, { to, name, vehicle, endsOn, daysLeft, tagId = "", priceInr = 249 }) {
  if (!isEmailConfigured(env)) {
    if (env.runtimeMode !== "production") {
      console.log(`
[ParkTag] Dev trial-ending mail for ${maskIdentifier(to)}: ${vehicle}, ${daysLeft} days
`);
      return;
    }
    throw new Error("Email is not configured on this server.");
  ...
  // The last notice reads differently from the first. Same facts, and the
  // urgency is real rather than manufactured: after this date the feature is
  // genuinely off.
  const urgent = daysLeft <= 1;
  const headline = urgent
    ? `Hi ${safeName}, premium on your ${safeVehicle} ends tomorrow`
    : `Hi ${safeName}, ${daysLeft} days of premium left on your ${safeVehicle}`;

  await transporter.sendMail({
    from: env.emailFrom || "ParkTag <noreply@parktag.me>",
    to,
    subject: urgent
      ? `Last day of premium on your ${vehicle || "vehicle"}`
      : `${daysLeft} days left on your ParkTag premium`,
    html: shell(env, {
      preheader: `After ${endsOn}, masked calls switch off and your vault drops to 3 documents.`,
      title: headline,
      body: `
        <p style="margin:0 0 18px;color:${BRAND.body};line-height:1.6;font-size:15px">Your complimentary premium year runs until <strong>${safeEnds}</strong>. Here is what changes the day after.</p>
        <div style="background:${BRAND.tintBg};border-left:4px solid ${BRAND.red};border-radius:10px;padding:16px 18px">
          <p style="margin:0 0 8px;color:${BRAND.tintInk};font-size:15px;line-height:1.5"><strong>Masked calls switch off.</strong> Someone standing at your car can no longer reach you by phone through the tag.</p>
          <p style="margin:0;color:${BRAND.tintInk};font-size:15px;line-height:1.5"><strong>Your vault drops to 3 documents.</strong> Nothing is deleted, but you cannot add more.</p>
        </div>
        <p style="margin:18px 0 0;color:${BRAND.body};line-height:1.6;font-size:15px">Keeping both costs <strong>Rs ${priceInr} for a year</strong>, which is about Rs ${Math.round(priceInr / 12)} a month.</p>
        ${button(`${appBase(env)}/owner-membership${tagId ? `?tag=${tagId}` : ""}`, "Keep premium on")}
        <p style="margin:14px 0 0;color:#8A97AB;font-size:13px;text-align:center">Your QR code and your stored documents stay yours either way.</p>
      `
    })
  });
}
